...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const { chat_session_id, messages } = requestBody;

        if (!chat_session_id || !messages || !Array.isArray(messages)) {
        ...
        // Convert messages to GROQ format and add system prompt at top
        const groqMessages = [
            {
                role: "system",
            ...
            {
                role: "user",
                content: "I am back in the chat. Please pick up the conversation where we left off."
            }
        ];
        ...
        console.log("Inserting assistant message to database...");
        const { data: insertedMessage, error: msgErr } = await db.from("chat_message").insert({
            session_id: chat_session_id,
            role: "assistant",
            content: aiResponse
        }).select("id").single();

        console.log("Message insertion result:", { hasMessage: !!insertedMessage, messageId: insertedMessage?.id, hasError: !!msgErr, error: msgErr });

        if (msgErr) {
            console.log("Message insertion failed:", msgErr);
            return new Response(JSON.stringify({ error: "Failed to save message to database" }), {
                status: 500,
                headers: { "content-type": "application/json", ...CORS_HEADERS }
            });
        }
